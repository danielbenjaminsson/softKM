 ...
 *
 * Modified for softKM by removing keyboard device addon dependency.
 */
...
// Define locally since we removed KeyboardInputDevice.h dependency
static const uint32 SYSTEM_SHUTTING_DOWN = 'SYsd';
...
TeamMonitorWindow::~TeamMonitorWindow()
{
	...
	if (gTeamMonitorWindow == this)
		gTeamMonitorWindow = NULL;
}
...
{
	Disable();
	// For softKM, allow the window to close when Cancel is pressed
	return true;
}
